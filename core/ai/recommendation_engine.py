...
class RecommendationEngine:
    ...
    def get_user_behavior_profile(self, user_id: int) -> Dict[str, Any]:
        ...
        The legacy TopTheme model has been removed, so we derive lightweight metrics
        from CSV analyses and theme requests to keep the interface stable for callers.
        """
        try:
            user = self.db.query(User).filter(User.id == user_id).first()

            analyses = (
                self.db.query(CSVAnalysis)
                .filter(CSVAnalysis.user_id == user_id)
                .order_by(desc(CSVAnalysis.created_at))
                .all()
            )
            total_analyses = len(analyses)
            last_analysis_at = analyses[0].created_at if analyses else None

            # Aggregate theme preferences from theme requests
            theme_preferences: Dict[str, int] = {}
            successful_themes = 0
            success_statuses = {"APPROVED", "COMPLETED", "FULFILLED", "SUCCESS"}
            ...
            for request in theme_requests:
                theme_preferences[request.theme_name] = (
                    theme_preferences.get(request.theme_name, 0) + 1
                )
                if request.status and request.status.upper() in success_statuses:
                    successful_themes += 1
            ...
            print(f"Error building user behavior profile: {e}")
            return {
                "user_id": user_id,
    ...
    def get_success_patterns(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            total_requests = sum(theme_preferences.values())
            top_theme = max(theme_preferences, key=theme_preferences.get)

            patterns = {
                "top_theme": top_theme,
                "total_requests": total_requests,
                "unique_themes": len(theme_preferences),
            }

            return {
                "patterns": patterns,
                "message": "Аналитика предпочтений пользователя успешно собрана",
            }
